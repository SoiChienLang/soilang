...
void tankPlayer::drawConsoleTank(int frame[][200])
{
    gotoxy(1,1);
    cout<<pos.X<<" "<<pos.Y;
	if (GetAsyncKeyState(VK_UP)&&pos.Y>minY&&checkFrame(frame,pos.Y-1,pos.X)==0){ direct = 1; setImageUp(); eraseConsole(); pos.Y--;  }
	if (GetAsyncKeyState(VK_DOWN)&&pos.Y<maxY&&checkFrame(frame,pos.Y+1,pos.X)==0) { direct = -1; setImageDown(); eraseConsole(); pos.Y++; }
	if (GetAsyncKeyState(VK_LEFT)&&pos.X>minX&&checkFrame(frame,pos.Y,pos.X-1)==0) { direct = -2; setImageLeft(); eraseConsole(); pos.X--; }
	if (GetAsyncKeyState(VK_RIGHT)&&pos.X<maxX&&checkFrame(frame,pos.Y,pos.X+1)==0) { direct = 2; setImageRight(); eraseConsole(); pos.X++; }
	drawConsole();
	check=0;
}

int tankPlayer::checkFrame(int frame[][200],short x,short y)
{
    for(int i=0;i<8;i++)
    for(int j=0;j<11;j++)
    {
