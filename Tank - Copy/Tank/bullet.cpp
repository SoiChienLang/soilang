...
{
    DWORD garbage = 0;
	if(direct==1)       FillConsoleOutputCharacter(hOut, 30, 1, pos, &garbage);
	else if(direct==-1) FillConsoleOutputCharacter(hOut, 31, 1, pos, &garbage);
	else if(direct==2)  FillConsoleOutputCharacter(hOut, 16, 1, pos, &garbage);
	else if(direct==-2) FillConsoleOutputCharacter(hOut, 17, 1, pos, &garbage);
	FillConsoleOutputAttribute(hOut, LIGHT_AQUA*16+BLACK, 1, pos, &garbage);
}
