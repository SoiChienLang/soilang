...
void menu::renderMode(SHORT xScreenSize,SHORT yScreenSize,const COORD origin)
{
    ...
    FillConsoleOutputCharacter(outConsole, 42, xScreenSize+2, { SHORT(origin.X-1),SHORT(origin.Y-1)}, &garbage);
    FillConsoleOutputAttribute(outConsole, 13, xScreenSize+2, { SHORT(origin.X-1),SHORT(origin.Y-1)}, &garbage);
    FillConsoleOutputCharacter(outConsole, 42, xScreenSize, { SHORT(origin.X),SHORT(origin.Y+yScreenSize)}, &garbage);
    FillConsoleOutputAttribute(outConsole, 13, xScreenSize, { SHORT(origin.X),SHORT(origin.Y+yScreenSize)}, &garbage);

    for(SHORT i=0;i<yScreenSize+1;i++){
        FillConsoleOutputCharacter(outConsole, 42, 1,{ SHORT(origin.X-1),SHORT(origin.Y+i)} , &garbage);
        FillConsoleOutputAttribute(outConsole, 13, 1, { SHORT(origin.X-1),SHORT(origin.Y+i)}, &garbage);
        FillConsoleOutputCharacter(outConsole, 42, 1,{ SHORT(origin.X+xScreenSize),SHORT(origin.Y+i)} , &garbage);
        FillConsoleOutputAttribute(outConsole, 13, 1, {SHORT (origin.X+xScreenSize),SHORT(origin.Y+i)}, &garbage);
    }
