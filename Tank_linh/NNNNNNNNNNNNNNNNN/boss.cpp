...
void boss::draw(SHORT i,frame &f)
{
	...
	time++;
	if(time == 10) time = 0;*/
	if(pos.X>maxX-35) direct=-2;
	else if(pos.X<minX+5) direct =2;
	if (direct == -2&&pos.X>minX+1) {  setImage(); eraseConsole(); pos.X-=3; }
    else if (direct == 2&&pos.X<maxX) {  setImage(); eraseConsole(); pos.X+=3; }
    drawConsole();
