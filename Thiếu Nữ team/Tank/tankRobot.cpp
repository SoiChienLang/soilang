...
void tankRobot::ai(tankPlayer &t, frame &f)
{
	if(pos.Y > t.pos.Y&&pos.Y>minY+1&&checkFrame(f,pos.X,pos.Y-1)==0) {
		setImageUp(); eraseConsole(); pos.Y--;
	}
	else if(pos.Y < t.pos.Y&&pos.Y<maxY&&checkFrame(f,pos.X,pos.Y+1)==0) {
		setImageUp(); eraseConsole(); pos.Y++;
	}
	else if(pos.X > t.pos.X&&pos.X>minX+1&&checkFrame(f,pos.X-1,pos.Y)==0) {
		setImageUp(); eraseConsole(); pos.X--;
	}
	else if(pos.X < t.pos.X&&pos.X<maxX&&checkFrame(f,pos.X+1,pos.Y)==0) {
		setImageUp(); eraseConsole(); pos.X++;
	}
} 
