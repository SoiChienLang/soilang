...
void tankRobot::draw(int i,frame &f)
{
    static int _time=0;
    srand(time(0));
    direct =(rand()%i)%5-2;
...
int tankRobot::checkFrame(frame &f, short x,short y)
{
...
int tankRobot::checkLocal(short x, short y)
{
...
void tankRobot::setCoord(int index)
{
    for(int i=0;i<7;i++){if(index%7==i) {pos.X=5+19*i;pos.Y=3;}}
}
void tankRobot::fire()
{
    static int time=0;
