...
void tankRobot::drawConsoleTank(int i, tankPlayer &p)
{
    srand(time(0));
    direct =(rand()%i)%5-2;
    if (direct == 1&&pos.Y>minY) {  setImageUp(); eraseConsole(); pos.Y--; }
    else if (direct ==-1&&pos.Y<maxY) {  setImageDown(); eraseConsole(); pos.Y++; }
    else if (direct == -2&&pos.X>minX) {  setImageLeft(); eraseConsole(); pos.X--; }
    else if (direct == 2&&pos.X<maxX) {  setImageRight(); eraseConsole(); pos.X++; }
    drawConsole();
}
